# iRacing SDK — Practice Telemetry Capture Profile

Version: 1.0 | 2026-09-08 | Implementation specification for Codex

## Purpose and implementation authority

This document defines the reduced telemetry profile for the practice-focused race-engineer platform. **The explicit whitelist in Sections 1–7 is the implementation authority for the current collector.** The original complete reference is preserved in Appendix A for future expansion. Do not interpret the appendix as an instruction to capture all fields.

The original verified inventory contains 334 SDK variable headers from the September 6, 2026 validation session. This profile selects a subset of those exact names. The inventory is session-specific; a field absent from a future runtime must be recorded as unavailable, not replaced with zero. Preserve native units, types, array counts, and source metadata. Do not invent unsupported channels.

### Capture policy

- Read the actual SDK header tick rate and enumerate the runtime catalog on connection.
- Capture only the selected live fields below. Preserve the original field names and array shapes; `[6]` denotes the declared six-element high-rate array, not six independent header names.
- Store the complete raw SessionInfo YAML whenever its update counter changes, without filtering any section.
- Preserve complete raw CarSetup snapshots at stint start and after detected pit entries, as in the original implementation. Keep the normalized setup inventory and original raw values.
- Retain the full discovered header catalog as metadata, even though only selected values are recorded. Mark excluded fields as `excluded_by_profile`, not `not_available`.
- Do not collect optional fields from the earlier review or the excluded Sections 8–12 in the current practice profile.
- Keep the original extensive reference available for future profile versions. No setup-write or `.sto` generation capability is implied by this capture specification.

## 1. Session, timing, and lap data

Keep only the previously selected Keep fields. The optional lap-delta families, rolling-N-lap fields, joker/race/pace extras, and replay channels are excluded.

**Included SDK fields:**

```text
SessionTick, SessionTime, SessionNum, SessionState, SessionUniqueID, SessionFlags, SessionTimeRemain, SessionTimeTotal, SessionTimeOfDay, SessionLapsRemain, SessionLapsRemainEx, SessionLapsTotal, IsOnTrack, IsOnTrackCar, IsInGarage, IsGarageVisible, OnPitRoad, Lap, LapCompleted, LapDist, LapDistPct, LapCurrentLapTime, LapLastLapTime, LapBestLapTime, LapBestLap
```

These fields provide run identity, session state, lap timing, and distance alignment. Derived lap deltas should be calculated by the analytics layer when needed.

## 2. Driver inputs and motion

**Included SDK fields:**

```text
Throttle, Brake, Clutch, SteeringWheelAngle, Speed, RPM, Gear, LatAccel, LongAccel, VertAccel, Roll, Pitch, Yaw, YawNorth, RollRate, PitchRate, YawRate, VelocityX, VelocityY, VelocityZ, SteeringWheelTorque
```

Use these for driver-input traces, braking and throttle analysis, steering demand, vehicle rotation, and motion. Do not present yaw rate alone as a direct understeer measurement.

## 3. High-rate advanced analytics

**Included SDK fields:**

```text
LatAccel_ST[6], LongAccel_ST[6], VertAccel_ST[6], RollRate_ST[6], PitchRate_ST[6], YawRate_ST[6], VelocityX_ST[6], VelocityY_ST[6], VelocityZ_ST[6]
```

Retain all nine selected high-rate arrays for advanced dynamics analysis. `SteeringWheelTorque_ST[6]` remains excluded because it was optional in the previous review.

## 4. Four-corner suspension, tires and brakes

**Included SDK fields:**

```text
LFbrakeLinePress, LFcoldPressure, LFodometer, LFshockDefl, LFshockDefl_ST[6], LFshockVel, LFshockVel_ST[6], LFtempCL, LFtempCM, LFtempCR, LFwearL, LFwearM, LFwearR, LFTiresAvailable, LFTiresUsed, RFbrakeLinePress, RFcoldPressure, RFodometer, RFshockDefl, RFshockDefl_ST[6], RFshockVel, RFshockVel_ST[6], RFtempCL, RFtempCM, RFtempCR, RFwearL, RFwearM, RFwearR, RFTiresAvailable, RFTiresUsed, LRbrakeLinePress, LRcoldPressure, LRodometer, LRshockDefl, LRshockDefl_ST[6], LRshockVel, LRshockVel_ST[6], LRtempCL, LRtempCM, LRtempCR, LRwearL, LRwearM, LRwearR, LRTiresAvailable, LRTiresUsed, RRbrakeLinePress, RRcoldPressure, RRodometer, RRshockDefl, RRshockDefl_ST[6], RRshockVel, RRshockVel_ST[6], RRtempCL, RRtempCM, RRtempCR, RRwearL, RRwearM, RRwearR, RRTiresAvailable, RRTiresUsed
```

All 15 fields for each of LF, RF, LR, and RR are included, including tire-set counts and odometers. The source does not verify direct wheel speed, tire slip angle, tire load, brake temperature, or live ride-height channels; do not add those as measured fields.
...
```text
FuelLevel, FuelLevelPct, FuelUsePerHour, RPM, Gear, EngineWarnings
```

RPM and Gear are shared with Section 2 and appear only once in the unique whitelist.
...
```text
AirTemp, TrackTemp, TrackTempCrew, RelativeHumidity, TrackWetness, Precipitation, WeatherDeclaredWet
```

All previously optional weather fields are excluded.

## 7. In-car adjustments

**Included SDK fields:**

```text
dcABS, dcBrakeBias, dcTractionControl, BrakeABSactive
```

Retain these active electronic settings and ABS activity. Other dashboard, wiper, starter, and control-toggle channels are excluded.

## 8–12. Excluded from the current practice profile

The following entire sections from the previous review are excluded from live value capture for now:

- Section 8: Pit service, tire sets, and strategy. The four-corner tire-set fields explicitly selected in Section 4 remain included; full raw SessionInfo and CarSetup snapshots remain unchanged.
- Section 9: Competitor arrays, including all `CarIdx*` fields.
- Section 10: Player race state, incidents, and player/competitor proximity fields. Session and lap identity fields explicitly selected in Section 1 remain included.
- Section 11: Camera, radio, replay, and simulator diagnostics.
- Section 12: Tire rumble-pitch feedback channels.

The excluded fields are retained in the runtime metadata catalog and the historical reference, not written as per-tick telemetry values. This is a profile decision, not a claim that the SDK does not expose them.

## 13. SessionInfo and CarSetup — unchanged

Retain the original complete-capture policy for SessionInfo and CarSetup. Store the entire raw YAML tree, including all available top-level and nested sections such as WeekendInfo, SessionInfo, DriverInfo, CarSetup, CameraInfo, RadioInfo, SplitTimeInfo, and QualifyResultsInfo. Do not remove camera, radio, competitor, pit, or other metadata from the raw YAML merely because their live telemetry equivalents are excluded.

Continue to capture every actual CarSetup leaf and preserve unknown or car-specific settings. Maintain the normalized setup schema, raw values, units, setup hashes, simulator build, car/track identity, fuel and tire context, setup changes, driver feedback, and validation status described in the original reference. The actual Corvette setup parameters must be discovered from a real snapshot; the broad setup categories in the reference are not a verified flat schema.

## Codex implementation instructions

Update the existing collector rather than replacing the SDK discovery mechanism. Use the exact whitelist in `practice_profile.json` (included as a companion file) as the selected live-value set. Do not use the appendix, prefix matching, or the old capture-all requirement to expand that set. Preserve full header discovery and full raw YAML capture independently of the live-value whitelist.

1. Load the versioned profile and validate that selected names are unique.
2. On connection, enumerate every runtime header and export the complete metadata catalog.
3. Resolve selected names against actual headers. Report missing selected fields as unavailable; do not fail the entire recording because a car lacks a field.
4. Read only selected values on each unique SDK tick, preserving native types, units, and declared array counts.
5. Write a profile identifier, profile version, simulator build, and catalog reference into recording metadata.
6. Preserve raw SessionInfo on update-counter changes and complete CarSetup snapshots at the existing trigger points.
7. Do not silently reintroduce excluded fields through broad prefix rules or the original capture-all implementation.
8. Keep unknown future headers in the metadata catalog but do not record their values until explicitly added to a new profile version.
9. Test the whitelist against the supplied 334-field inventory and synthetic headers, including missing fields and six-element arrays.
10. Keep setup generation and `.sto` validation as separate future subsystems.

### Acceptance criteria

- The live-value whitelist contains exactly the unique names listed in Sections 1–7.
- Every selected name exists in the September 6 verified inventory.
- No excluded field is written to the current profile's per-tick telemetry stream.
- The complete runtime header catalog and complete raw SessionInfo/CarSetup data remain available.
- Arrays retain their declared count and are not flattened into fabricated SDK names.
- Missing selected fields are distinguishable from zero-valued fields.
- A future profile can add fields without modifying or losing historical raw metadata.

### Profile counts

- Original verified live headers: **334**.
- Selected unique live headers: **130**.
- Excluded live headers: **204**.
- Full raw SessionInfo and CarSetup: **unchanged**.

## Appendix A — Original extensive reference (preserved)

The following is the original reference, retained for future discovery, additional analytics, and profile expansion. Its original capture-all instructions are historical context and are superseded by the explicit practice-profile whitelist above for the current implementation.
